  ...
  return <form action={formAction} className="grid gap-6">
    ...
    <FormSection title="Date de autentificare" description="Emailul identifică utilizatorul. Modificările sensibile revocă automat sesiunile active.">
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className={`${labelClass} md:col-span-2`}>{editing ? "Parolă nouă" : "Parolă inițială"}<input name="newPassword" type="password" minLength={14} maxLength={128} autoComplete="new-password" className={fieldClass} /><span className="mt-1.5 block text-xs font-normal leading-5 text-muted">Minimum 14 caractere, cu literă mare, literă mică și cifră. {editing ? "Lasă gol pentru a păstra parola actuală." : "Nu este necesară dacă trimiți invitație."}</span><FieldError errors={errors.newPassword} /></label>
