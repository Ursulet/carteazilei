    ...
    <>
      ...
      <section className="pb-16 md:pb-24 lg:pb-28">
        <div className="mx-auto grid w-full max-w-7xl gap-14 px-5 sm:px-6 lg:px-8">
          {groups.length ? groups.map(([month, entries]) => <section key={month}><h2 className="border-b border-border pb-4 font-display text-3xl font-semibold capitalize">{monthHeading(month)}</h2><div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{entries.map((feature) => <article key={feature.id} className="overflow-hidden rounded-2xl border border-border bg-surface p-5 transition hover:-translate-y-0.5 hover:border-accent hover:shadow-sm"><Link href={`/cartea-zilei/${feature.featureDate}`} className="block"><BookCover cover={feature.cover} title={feature.book.title} className="mx-auto max-w-[11rem]" /><p className="mt-5 text-xs font-bold uppercase tracking-wide text-accent-dark">{formatEditorialDate(feature.featureDate)}</p><h3 className="mt-2 font-display text-2xl font-semibold leading-tight">{feature.book.title}</h3></Link><p className="mt-1 text-sm text-muted">de <AuthorLink name={feature.author.name} slug={feature.author.slug} /></p><p className="mt-4 line-clamp-3 text-sm leading-6 text-muted">{feature.whyToday}</p></article>)}</div></section>) : <div className="rounded-2xl border border-dashed border-border bg-surface px-6 py-14 text-center"><h2 className="font-display text-3xl font-semibold">Nu există selecții pentru aceste filtre.</h2><p className="mt-3 text-sm text-muted">Schimbă filtrele sau revino la arhiva completă.</p><Link href="/cartea-zilei/arhiva" className="mt-6 inline-flex min-h-11 items-center rounded-full border border-border px-5 text-sm font-bold hover:border-brand">Vezi toată arhiva</Link></div>}
        </div>
      </section>
    </>
  );
