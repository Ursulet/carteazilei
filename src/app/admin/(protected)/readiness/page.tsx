...
export const metadata: Metadata = { title: "Calitatea catalogului" };
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        title="Calitatea catalogului"
        description="Vezi într-un singur loc ce lipsește din cărți, calendar și paginile SEO și deschide direct elementul care trebuie corectat."
      />
      <section className="rounded-2xl border border-border bg-surface p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="font-display text-2xl font-semibold">Filtre</h2>
            <p className="mt-1 text-sm text-muted">O ofertă trebuie reverificată după {report.staleOfferDays} de zile.</p>
          </div>
      ...
      {showSeo ? (
        <section className="mt-8 rounded-2xl border border-border bg-surface p-5 sm:p-6">
          ...
          <h2 className="mt-2 font-display text-2xl font-semibold">Pagini SEO care trebuie completate ({report.seo.belowGate.length})</h2>
          ...
          ) : <p className="mt-5 text-sm font-semibold text-brand">Toate paginile SEO cerute pentru indexare sunt complete.</p>}
        </section>
      ) : null}

      <p className="mt-6 text-xs leading-5 text-muted">Actualizat la {new Intl.DateTimeFormat("ro-RO", { dateStyle: "medium", timeStyle: "short" }).format(report.generatedAt)}.</p>
    </div>
  );
}
