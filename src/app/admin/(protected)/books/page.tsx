...
import { getAdminBookGenreOptions, getAdminBooks } from "@/db/queries/admin-editorial";
import { hasPermission } from "@/lib/auth/permissions";
import { requireSectionAccess } from "@/lib/auth/principal";

import { bulkAddBookGenreAction, bulkUpdateBookStatusAction, deleteBookAction } from "./actions";
...
export default async function BooksPage() {
  ...
  const [rows, genreOptions] = await Promise.all([getAdminBooks(), getAdminBookGenreOptions()]);
  return (
    <>
      ...
      {rows.length === 0 ? <EmptyState>Nu există încă nicio carte. Creează autorul, apoi prima înregistrare editorială.</EmptyState> : (
        <>
        {canBatch ? <BulkBookStatusToolbar action={bulkUpdateBookStatusAction} total={rows.length} canPublish={canPublish} canDraft={canUpdate} genreAction={canUpdate ? bulkAddBookGenreAction : undefined} genres={canUpdate ? genreOptions : []} /> : null}
