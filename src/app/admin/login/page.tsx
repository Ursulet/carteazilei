...
export default async function AdminLoginPage({
  ...
  searchParams: Promise<{ callbackUrl?: string; "cont-actualizat"?: string; invitatie?: string }>;
}) {
  const [principal, settings] = await Promise.all([getInternalPrincipal(), getPublicSiteSettings()]);
      ...
      <div className="w-full max-w-md">
        ...
        <section className="rounded-3xl border border-border bg-surface p-6 shadow-[0_20px_60px_rgba(23,21,18,0.08)] sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-muted">
            Această zonă este rezervată echipei {settings.siteName}.
          </p>
          {query["cont-actualizat"] === "1" ? (
            <p role="status" className="mt-5 rounded-xl border border-brand/20 bg-accent-soft px-4 py-3 text-sm font-semibold text-brand">
              Contul a fost actualizat. Autentifică-te din nou cu noile date.
            </p>
          ) : null}
          {query.invitatie === "acceptata" ? <p role="status" className="mt-5 rounded-xl border border-brand/20 bg-accent-soft px-4 py-3 text-sm font-semibold text-brand">Contul a fost activat. Te poți autentifica.</p> : null}
