...
export default async function AuthorsPage() {
  ...
  return <><AdminPageHeader eyebrow="Catalog" title="Autori" description="Profiluri publice, fapte verificate și note de sursă păstrate separat." action={{ href: "/admin/authors/new", label: "Autor nou" }} />{rows.length === 0 ? <EmptyState>Nu există încă autori.</EmptyState> : <div className="overflow-x-auto rounded-2xl border border-border bg-surface shadow-sm"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-border bg-paper text-xs uppercase tracking-wide text-muted"><tr><th className="px-5 py-4">Autor</th><th className="px-5 py-4">Slug</th><th className="px-5 py-4">Stare</th><th className="px-5 py-4">Actualizat</th><th className="px-5 py-4"><span className="sr-only">Acțiuni</span></th></tr></thead><tbody>{rows.map((author) => <tr key={author.id} className="border-b border-border/70 last:border-0"><td className="px-5 py-4"><Link href={`/admin/authors/${author.id}`} className="font-bold underline decoration-border underline-offset-4 hover:decoration-brand">{author.name}</Link></td><td className="px-5 py-4 text-muted">{author.slug}</td><td className="px-5 py-4"><StatusBadge status={author.status} /></td><td className="px-5 py-4 text-muted">{new Intl.DateTimeFormat("ro-RO", { dateStyle: "medium", timeStyle: "short" }).format(author.updatedAt)}</td><td className="px-5 py-4"><ConfirmDeleteForm action={deleteAuthorAction.bind(null, author.id)} /></td></tr>)}</tbody></table></div>}</>;
}
