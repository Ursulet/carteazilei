...
export default async function HomePage() {
  ...
  return (
    <>
      <section className="overflow-hidden py-16 md:py-24 lg:py-28">
        <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-5 sm:px-6 lg:grid-cols-12 lg:gap-16 lg:px-8">
          <div className="lg:col-span-7">
            ...
            <p className="mt-5 text-xs font-semibold text-muted">Recomandări explicate · selecție editorială · afiliere transparentă</p>
          </div>
      ...
      <section className="py-16 md:py-24 lg:py-28">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="rounded-[2rem] border border-border bg-surface p-7 shadow-sm sm:p-10 lg:flex lg:items-center lg:justify-between lg:gap-12 lg:p-14">
            <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-dark">Recomandare personalizată</p><h2 className="mt-3 font-display text-4xl font-semibold tracking-[-0.03em] sm:text-5xl">Nu știi ce să citești?</h2><p className="mt-5 text-lg leading-8 text-muted">Răspunde la câteva întrebări. Noi alegem una.</p><p className="mt-3 text-sm font-semibold text-muted">2–3 minute · fără cont obligatoriu</p></div>
      ...
      {hasTaxonomyDiscovery ? <section className="py-16 md:py-24 lg:py-28"><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8"><SectionHeading eyebrow="Colecții editoriale" title="Explorează după ce cauți" description="Genuri și audiențe publicate numai după verificare editorială." /><div className="mt-10 grid gap-8 lg:grid-cols-2">{discovery.genres.length ? <div><h3 className="text-sm font-bold uppercase tracking-wide text-muted">Gen</h3><div className="mt-4 flex flex-wrap gap-3">{discovery.genres.map((genre) => <Link key={genre.slug} href={`/carti/gen/${genre.slug}`} className="rounded-full border border-border bg-surface px-4 py-2.5 text-sm font-bold transition hover:border-brand hover:bg-accent-soft">{genre.name}</Link>)}</div></div> : null}{discovery.audiences.length ? <div><h3 className="text-sm font-bold uppercase tracking-wide text-muted">Pentru cine</h3><div className="mt-4 flex flex-wrap gap-3">{discovery.audiences.map((audience) => <Link key={audience.slug} href={`/carti/pentru/${audience.slug}`} className="rounded-full border border-border bg-surface px-4 py-2.5 text-sm font-bold transition hover:border-brand hover:bg-accent-soft">{audience.name}</Link>)}</div></div> : null}</div></div></section> : null}

      {discovery.nextReads.length ? <section className="border-y border-border bg-surface py-16 md:py-24 lg:py-28"><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8"><SectionHeading eyebrow="Continuă lectura" title="Ce să citești după…" description="Puncte de plecare editoriale construite din relații aprobate, nu doar din etichete comune." /><div className="mt-10 grid gap-4 md:grid-cols-2">{discovery.nextReads.map((item) => <Link key={item.slug} href={`/ce-sa-citesc-dupa/${item.slug}`} className="group rounded-2xl border border-border bg-paper p-6 transition hover:-translate-y-0.5 hover:border-accent hover:shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-accent-dark">După {item.author}</p><h3 className="mt-3 font-display text-2xl font-semibold">Ce să citești după {item.title}</h3><span className="mt-5 inline-flex items-center text-sm font-bold text-brand">Vezi traseul editorial<ArrowRight aria-hidden="true" className="ms-2 size-4" /></span></Link>)}</div></div></section> : null}
      ...
      <section className="border-t border-border bg-paper py-16 md:py-24 lg:py-28">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8"><SectionHeading eyebrow="Încredere editorială" title="Cum alegem cărțile" description="O recomandare bună spune și de ce funcționează, și unde s-ar putea să nu fie potrivită." /><div className="mt-10 grid gap-5 md:grid-cols-3">{[{ icon: Compass, title: "Evaluare editorială", text: "Fiecare selecție are un editor asumat, argumente și o rezervă sinceră." }, { icon: ShieldCheck, title: "Potrivire, nu popularitate", text: "Contextul cititorului contează mai mult decât topurile sau volumul catalogului." }, { icon: BookHeart, title: "Monetizare separată", text: "Afilierea poate susține proiectul, dar nu schimbă cartea recomandată." }].map((item) => { const Icon = item.icon; return <article key={item.title} className="rounded-2xl border border-border bg-surface p-6"><Icon aria-hidden="true" className="size-5 text-accent-dark" /><h3 className="mt-5 font-display text-2xl font-semibold">{item.title}</h3><p className="mt-3 text-sm leading-6 text-muted">{item.text}</p></article>; })}</div><div className="mt-8"><ButtonLink href="/cum-recomandam" variant="secondary">Vezi metodologia</ButtonLink></div></div>
      </section>
    </>
  );
}
