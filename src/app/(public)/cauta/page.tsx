        ...
        <section aria-labelledby="search-books-heading">
          ...
          <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {results.books.map((book) => (
              <article key={book.id} className="rounded-2xl border border-border bg-surface p-5 transition hover:-translate-y-0.5 hover:border-accent hover:shadow-sm">
                <Link href={`/carte/${book.slug}`} className="group block">
                  ...
                  <span className="mt-5 inline-flex text-sm font-bold text-brand">Vezi analiza →</span>
                </Link>
              </article>
            ))}
          </div>
