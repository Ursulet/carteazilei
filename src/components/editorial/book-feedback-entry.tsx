...
export function BookFeedbackEntry({ slug }: { slug: string }) {
  return (
    <section className="rounded-[2rem] border border-border bg-brand p-7 text-white sm:p-10">
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/60">Feedback editorial</p>
      <h2 className="mt-3 font-display text-3xl font-semibold">Ai citit-o?</h2>
      <p className="mt-4 max-w-2xl leading-7 text-white/70">Spune-ne dacă analiza te-a ajutat și ce ți s-a părut diferit. Feedbackul nu schimbă automat verdictul editorial.</p>
